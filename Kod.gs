...
function parsePrompt_(categories, hints, nFiles) {
  ...
  return [
    ...
    '- Bon/kwit za zwrot opakowań użyty przy płatności (np. Biedronka "Inna: Bon - zwrot opak", Lidl "Zwrot kaucji") to też pozycja "Kaucja" z v ujemnym,',
    '  a total to wtedy kwota PO odjęciu bonu (to, co zapłacono gotówką/kartą), nie "Suma PLN" sprzed bonu.',
    '- Rok w dacie czytaj uważnie: dziś jest ' + Utilities.formatDate(new Date(), TZ, 'yyyy-MM-dd') + ', paragony są zwykle z ostatnich tygodni — jeśli wychodzi inny rok, sprawdź jeszcze raz i napisz o tym w warnings.',
